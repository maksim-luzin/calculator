...
const mutations = {
  [MutationTypes.AddNewDigit](state: IState, newDigit: string) {
    if (state.currentValue.length < MaxDigitCapacity) {
      if (newDigit !== ButtonNames.Null || state.currentValue !== ButtonNames.Null) {
        state.currentValue = state.currentValue.slice(0, state.currentValuePosition) + newDigit;
      }
      state.currentValuePosition = state.currentValue.length;
      state.currentCalculation = state.currentCalculation.slice(0, state.currentCalculationPosition);
    }
  },
  ...
  [MutationTypes.AddNewRecord](state: IState, newRecord: string) {
    if (state.currentValue) {
      ...
      state.currentCalculation = state.currentCalculation.slice(0, state.currentCalculationPosition - 1);
    }
  ...
  [MutationTypes.ChangeSign](state: IState) {
    const numberCurrentValue = parseFloat(state.currentValue.slice(0, state.currentValuePosition));
    state.currentValue = String(-1 * (isNaN(numberCurrentValue) ? 0 : numberCurrentValue));
    if (isNaN(numberCurrentValue) || !numberCurrentValue) state.isCurrentValueDot = false;
    if (Math.floor(numberCurrentValue) === numberCurrentValue) state.isCurrentValueDot = false;
    state.currentValuePosition = state.currentValue.length;
    state.currentValuePosition = state.currentValue.length;
    state.currentCalculation = state.currentCalculation.slice(0, state.currentCalculationPosition);
  },
  ...
  [MutationTypes.UndoRecord](state: IState) {
    state.currentCalculationPosition = Math.max(0, state.currentCalculationPosition - 2);
    const currentValue = state.currentCalculation[Math.max(0, state.currentCalculationPosition)];
  ...
  [MutationTypes.RedoRecord](state: IState) {
    state.currentCalculationPosition = Math.min(state.currentCalculation.length, state.currentCalculationPosition + 2);
    const currentValue = state.currentCalculation[state.currentCalculationPosition];
    if (isNaN(parseFloat(currentValue))) {
      state.currentValue = "";
      state.currentValuePosition = 0;
    } else {
  ...
  [MutationTypes.UndoDigit](state: IState) {
    ...
    state.isCurrentValueDot = hasDot(state.currentValue.slice(0, state.currentValuePosition));
  },
  [MutationTypes.RedoDigit](state: IState) {
    state.currentValuePosition = Math.min(state.currentValue.length, state.currentValuePosition + 1);
    state.isCurrentValueDot = hasDot(state.currentValue.slice(0, state.currentValuePosition));
  },
};
