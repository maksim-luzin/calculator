...
const getters = {
  ...
  getCurrentCalculation(state: IState) {
    return (
      state.currentCalculation.slice(0, state.currentCalculationPosition).join(" ") +
      " " +
      state.currentValue.slice(0, state.currentValuePosition)
    );
  },
  getLastCalculation({ currentCalculation, currentCalculationPosition }: IState) {
    return currentCalculation[currentCalculationPosition - 1] || "";
  },
  getComputedValue(state: IState) {
    const computedValue = state.currentCalculation.slice(0, state.currentCalculationPosition).reduce(
      (acc, elem) => {
    ...
    const lengthRound = String(Math.abs(Math.round(allComputedValue))).length - 1;

    return `${Math.round(allComputedValue / Math.pow(10, lengthRound - 3)) / 1000}e${lengthRound}`;
  },
